...
## Usage
This documnetation gives example for following features:

1. [Getting Started](#getting-started)
2. [Basic usage](#basic-usage)
3. [Different types of columns](#different-types-of-columns)
4. [Row and column resizing](#resizing)
5. [Drag and drop rows/columns](#drag-drop)
6. [Merge Rows and Columns](#merge)
7. [Nested Headers](#nested-headers)
8. [Pagination](#pagination)
9. [Search](#search)
10. [Excel formulas](#excel-formulas)
11. [Style](#style)
12. [Shiny Integration](#shiny-integartion)
...
library(excelR)
 ...
 columns = data.frame(title=c('Model', 'Availibility'),
                      width= c(300, 300),
...
#### Drag and drop rows/columns {#drag-drop}
```r
library(excelR)
...
library(excelR)
 ...
 nestedHeaders = list( data.frame(title=c("Plant Attribute"), colspan=c(5)), 
                       data.frame(title=c("Sepal Attributes", "Petal Attributes", "Species"), colspan=c(2, 2, 1)))
...
```r
 ...
 style = list(A1=c('background-color:orange', 'color:green'), B1=c('background-color:orange', 'color:green'))
                      
 excelTable(data=data, columns = columns, style = style)
```
![](inst/images/style.png)

### Shiny Integration {#shiny-integartion}
```r
  library(shiny)
  library(excelR)
